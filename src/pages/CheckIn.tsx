...
export default function CheckIn() {
  ...
  const buscarAgendamento = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('agendamentos').select('*').eq('cliente_cpf', cpfLimpo).eq('status', 'confirmado').single();
      if (error || !data) {
        toast({
  ...
  return <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-background to-muted">
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          {step === 'cpf' && <div className="space-y-4">
          ...
          {step === 'dados' && agendamento && <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={checkInData.loja} onValueChange={value => setCheckInData({
              ...checkInData,
              loja: value
            })}>
                  <SelectTrigger>
          ...
          {step === 'sucesso' && <div className="text-center space-y-4">
