/**
 * Teste da rota GET get-corretor-historico
 *
 * Uso:
 *   node scripts/test-historico-corretor.mjs
 *   node scripts/test-historico-corretor.mjs --integraId=UUID
...
const url = new URL(`${supabaseUrl}/functions/v1/get-corretor-historico`);
url.searchParams.set('integraId', args.integraId);
url.searchParams.set('limit', String(args.limit));
url.searchParams.set('offset', String(args.offset));

console.log('=== Teste get-corretor-historico ===\n');
...
if (response.ok) {
  ...
  console.log(`  Total finalizadas: ${data.total}`);
