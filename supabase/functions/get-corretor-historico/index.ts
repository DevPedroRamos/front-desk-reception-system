...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (countError) {
      console.error('Erro ao contar visitas:', countError);
      return jsonResponse({ error: 'Falha ao consultar historico', detail: countError.message }, 500);
    }
    ...
    if (error) {
      ...
      return jsonResponse({ error: 'Falha ao consultar historico', detail: error.message }, 500);
    }
    ...
    console.error('Erro inesperado em get-corretor-historico:', err);
